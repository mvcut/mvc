...
const videoTypes = [
  ...
  {name: '2160p (4k)', bitrate: 68, width: 3840, height: 2160},
...
window.addEventListener('drop', e => {
  ...
  if (files.length) {
    if (files[0].name.toLowerCase().endsWith('.mvc')) {
      loadFile(files[0]);
    } else {
      importFiles(files);
    }
...
const helpMenu = new Menu([
  {label: 'Contribute', fn: modal(document.getElementById('help-modal'))},
  {label: 'Shortcuts', fn: modal(document.getElementById('shortcuts-modal'))},
